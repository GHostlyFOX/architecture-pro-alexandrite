...
minikube image build -t billing:latest services/billing/
...
kubectl exec -it $(kubectl get pods -l app=orders -o jsonpath='{.items[0].metadata.name}') -- wget -qO- http://orders:8080
```

## Структура проекта
- `services/orders/` - Исходный код service-a
- `services/billing/` - Исходный код service-b  
